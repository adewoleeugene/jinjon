...
<style>
	...
	.studio { min-height:100vh; overflow:visible; background:var(--text); color:var(--bg); opacity:0; transform:translateY(28px); transition:opacity .9s ease,transform 1.1s cubic-bezier(.2,.8,.2,1); }
	.studio.visible { opacity:1; transform:none; }
	.studio-intro { position:relative; min-height:100svh; padding:clamp(1.5rem,3vw,3rem) clamp(1.25rem,5vw,5rem); display:flex; flex-direction:column; justify-content:space-between; background:var(--text); }
	.studio-intro::after { content:''; position:absolute; left:clamp(1.25rem,5vw,5rem); bottom:9%; width:23%; height:1px; background:var(--bg); transform:rotate(-8deg); transform-origin:left; }
	.intro-meta { display:flex; justify-content:space-between; border-top:1px solid var(--bg); padding-top:.7rem; font-family:'JetBrains Mono',monospace; font-size:.55rem; letter-spacing:.14em; text-transform:uppercase; }
	...
	.studio-stamp { position:absolute; right:5%; bottom:7%; width:110px; aspect-ratio:1; display:grid; place-content:center; border:2px solid var(--accent); border-radius:50%; transform:rotate(9deg); background:transparent; color:var(--bg); font-size:1.8rem; font-weight:800; line-height:.75; text-align:center; }
	.studio-stamp span { font-family:'JetBrains Mono',monospace; font-size:.45rem; letter-spacing:.18em; }

	.category-index { display:flex; justify-content:space-between; gap:2rem; padding:1.1rem clamp(1.25rem,5vw,5rem); border-block:1px solid var(--bg); background:var(--text); font-family:'JetBrains Mono',monospace; font-size:.55rem; letter-spacing:.1em; text-transform:uppercase; }
	.category-index div { display:flex; flex-wrap:wrap; justify-content:flex-end; gap:1.2rem; }
	.category-index a { color:var(--text-dim); }
	.category-index a:hover { color:var(--bg); }

	.category-stack { --stack-gap:clamp(1.65rem,2.4vw,2.5rem); position:relative; background:var(--bg); }
	...
	.category-panel { position:sticky; top:calc(var(--stack-index) * var(--stack-gap)); min-height:calc(100svh - (var(--stack-index) * var(--stack-gap))); display:grid; grid-template-columns:minmax(17rem,.62fr) minmax(0,1.38fr); overflow:hidden; border:1px solid var(--text-dim); border-bottom:0; border-radius:clamp(1.4rem,2.3vw,2.5rem) clamp(1.4rem,2.3vw,2.5rem) 0 0; box-shadow:0 -12px 34px rgba(0,0,0,.24); }
	.tone-0 { background:var(--text); color:var(--bg); }
	...
	.category-number,.category-eyebrow,.project-count { font-family:'JetBrains Mono',monospace; font-size:.53rem; letter-spacing:.12em; text-transform:uppercase; }
	.category-eyebrow { margin-top:auto; opacity:.6; }
	.category-copy h3 { max-width:8ch; margin:1rem 0 1.5rem; font-size:clamp(3.5rem,7vw,8rem); line-height:.82; letter-spacing:-.07em; }
	.category-description { max-width:38ch; font-size:clamp(.95rem,1.2vw,1.15rem); line-height:1.6; }
	...
	.tone-1 .project-meta>span { background:var(--text); color:var(--bg); }
	...
	@media(max-width:620px){
		...
		.category-stack { --stack-gap:1.35rem; }
